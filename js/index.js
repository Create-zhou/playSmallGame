...
var Main = {
	...
	initData: function () { //初始化数据
		WF.file.imgs(["img/man.png", "img/block.png", "img/move.png", "img/thorn.png", "img/flip.png", "img/thorn_bg.png"], function (imgs) {
			...
			Main.gameInfo.w = canvas.width;// 获取画布宽度
			Main.gameInfo.h = canvas.height;// 获取画布高度

			Main.cxt = canvas.getContext("2d");//获取画笔
			WF.getId("js_start_loading").style.display = "none";//加载中...隐藏
			WF.getId("js_start_btn").style.display = "block";//开始按钮  显示
		});
	},
	start: function () {//开始游戏

		WF.getId("js_start_flush").style.display = "none";
		//初始化人物 在离左边150 上面0创建一个小人
	...
	initBlock: function (imgs) {//初始化障碍物
		...
		var block = new NormalBlock(120, 600, imgs[1], Main.cxt, Main.gameInfo);
	...
	initEvent: function () {//初始化事件
		...
		WF.getId("js_main").onkeydown = Main.keyDown.bind(this);
		WF.getId("js_main").onkeyup = Main.keyUp.bind(this);
	...
	over: function () {
		...
		if (this.level >= 3) {
			WF.getId("js_end_flush").getElementsByTagName("p")[0].innerHTML = "你太厉害啦,下了<label>" + this.level + "</label>层,男人中的男人呀！";
